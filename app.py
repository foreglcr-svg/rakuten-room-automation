...
CATEGORY_RULES = [
    ...
    # 注: 「器」単体は蒸し器/加湿器/炊飯器まで誤爆するため入れない。器系は具体語で拾う。
    ("interior", ["マグカップ", "食器", "茶器", "うつわ", "皿", "プレート", "花瓶", "一輪挿し", "フラワーベース", "花器", "陶器", "磁器", "porcelain", "波佐見焼", "益子焼", "信楽焼", "グラス", "タンブラー", "ポスター", "アートフレーム", "オブジェ", "インテリア雑貨", "ダルトン", "dulton", "プエブコ", "puebco", "キャンドル", "お香", "インセンス", "アロマ", "ディフューザー", "フレグランス", "香水", "ルームミスト", "ドリッパー", "コーヒーミル", "ケメックス", "急須", "コーヒーサーバー", "フレンチプレス"]),
...
def fetch_candidates(config):
    ...
    for kw in todays_keywords(config):
        ...
        for sort in sorts:
            page = rng.randint(1, 4)  # 1ページ目の超定番だけでなく2〜4ページ目も掘る(候補を広げる)
...
def select_items(candidates, config, history):
    ...
    """
    # 直近history_window件だけ除外(古い商品は再登場OK)。全部除外し続けると新規が枯れるため。
    window = config.get("history_window")
    posted = set(history[-window:] if window else history)
